...
class CondUNet2DModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        Args:
            ...
            class_emb (`torch.FloatTensor`, *optional*, defaults to `None`):
                Optional class embeddings for conditioning. They will be summed with the timestep embeddings.
        ...
        """
        # checks
        if class_labels is not None and class_emb is not None:
            raise ValueError("Cannot specify both class_labels and class_emb")
        ...
        if self.class_embedding is not None:
            if class_labels is None and class_emb is None:
                raise ValueError(
                    "either class_labels or class_emb should be provided when doing class conditioning"
                )
